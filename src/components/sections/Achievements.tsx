...
export default function Achievements() {
  return (
    <section id="achievements" style={{ background: "var(--bg-primary)", padding: "120px 0" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 40px" }} className="ach-container">
        ...
        >
          ...
          <h2 style={{ fontSize: "clamp(32px, 4vw, 52px)", fontWeight: 800, letterSpacing: "-0.03em", color: "var(--text-100)", margin: 0, lineHeight: 1.05 }}>
            Credibility &amp;<br />
            <span style={{ color: "var(--text-20)" }}>recognition.</span>
          </h2>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }} className="ach-grid">
          ...
          {/* Certifications */}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.12em", color: "var(--text-40)", textTransform: "uppercase", marginBottom: 24 }}>
              Certifications
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid var(--border-a)", borderRadius: 14, overflow: "hidden" }}>
              {certifications.map((cert, i) => (
                <motion.div
                  ...
                  style={{
                    ...
                    padding: "20px 22px",
                    borderBottom: i < certifications.length - 1 ? "1px solid var(--border-a)" : "none",
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-80)", marginBottom: 3 }}>
                      {cert.name}
                    </div>
                    <div style={{ fontSize: 12, color: "var(--text-40)", marginBottom: cert.note ? 4 : 0 }}>{cert.issuer} · {cert.date}</div>
                    {cert.note && (
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 5 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          {item.type === "award" && <span style={{ fontSize: 16 }}>🏆</span>}
                          <div style={{ fontSize: 15, fontWeight: 600, color: item.type === "award" ? "#FCD34D" : "var(--text-100)", lineHeight: 1.4 }}>{item.title}</div>
                        </div>
                        <span style={{ fontSize: 12, color: "var(--text-40)", flexShrink: 0 }}>{item.year}</span>
                      </div>
                      <div style={{ fontSize: 12, color: item.type === "award" ? "rgba(251,191,36,0.6)" : "#3B82F6", marginBottom: 5, fontWeight: 500 }}>{item.org}</div>
                      <div style={{ fontSize: 13, color: "var(--text-60)", lineHeight: 1.6 }}>{item.description}</div>
                      {item.type === "award" && (
                        <a
