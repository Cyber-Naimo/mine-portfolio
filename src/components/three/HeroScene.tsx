...
const NODES: NodeDef[] = [
  { pos: [ 0.0,  0.0,  0.0], color: "#3B82F6", size: 0.22, pulse: 0.0 },
  { pos: [-2.2,  0.4, -0.5], color: "#10B981", size: 0.15, pulse: 1.1 },
  { pos: [ 2.0, -0.2, -0.4], color: "#10B981", size: 0.15, pulse: 2.3 },
  { pos: [ 0.3,  1.6, -0.3], color: "#10B981", size: 0.15, pulse: 0.7 },
  { pos: [-1.2, -1.3,  0.2], color: "#06B6D4", size: 0.10, pulse: 1.8 },
  { pos: [ 2.6,  0.8,  0.3], color: "#06B6D4", size: 0.10, pulse: 3.1 },
  { pos: [-2.8, -0.5,  0.4], color: "#a78bfa", size: 0.08, pulse: 0.4 },
  { pos: [ 1.0, -1.5, -0.1], color: "#a78bfa", size: 0.08, pulse: 2.6 },
  { pos: [-0.5,  0.3,  1.1], color: "#F59E0B", size: 0.09, pulse: 1.4 },
];
  ...
  return (
    <group ref={groupRef} position={pos}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color={c} transparent opacity={0.22} blending={THREE.AdditiveBlending} side={THREE.BackSide} depthWrite={false} />
      </mesh>
      <mesh>
        <sphereGeometry args={[size * 3.6, 12, 12]} />
        <meshBasicMaterial color={c} transparent opacity={0.07} blending={THREE.AdditiveBlending} side={THREE.BackSide} depthWrite={false} />
      </mesh>
    </group>
  );
}
...
function ConnectionLines() {
  ...
  return (
    <lineSegments geometry={geo}>
      <lineBasicMaterial color="#60A5FA" transparent opacity={0.14} blending={THREE.AdditiveBlending} depthWrite={false} />
    </lineSegments>
  );
}
...
function DataPackets() {
  ...
  const tVals = useMemo<Float32Array>(() => {
    ...
    EDGES.forEach((_, e) => { for (let p = 0; p < PPE; p++) arr[e * PPE + p] = p / PPE; });
    return arr;
  }, []);
  ...
  return (
    <points ref={ptsRef}>
      ...
      <pointsMaterial vertexColors size={0.065} sizeAttenuation transparent opacity={0.95} blending={THREE.AdditiveBlending} depthWrite={false} />
    </points>
  );
}
...
function StarDust() {
  ...
  return (
    <Points ref={ref} positions={positions} stride={3} frustumCulled={false}>
      <PointMaterial transparent size={0.016} sizeAttenuation color="#7B9FFF" opacity={0.38} depthWrite={false} />
    </Points>
  );
}
...
export default function HeroScene() {
  return (
    <div className="absolute inset-0" aria-hidden="true">
      <Canvas camera={{ position: [0, 0, 6.5], fov: 58 }} gl={{ antialias: true, alpha: true }} dpr={[1, 1.5]}>
        <StarDust />
        <KubeNetwork />
      </Canvas>
      {/* Vignette uses CSS var — matches bg in both themes */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 95% 95% at 50% 50%, transparent 50%, var(--vignette) 100%)" }}
      />
    </div>
  );
}
